/* === Triple Triad - Game Logic (v17 - Multiplayer) === */
...
async function playCard(owner, handIndex, cellIndex) {
  ...
  // ONLINE: Enviar jogada se for minha vez e eu for humano
  if (state.isOnline && owner === "you") {
    state.matchmaking.sendMove(handIndex, cellIndex, card);
  }
  ...
  if (isGameOver()) { await sleep(500); endGame(); }
    ...
    state.busy = false; updateHandInteractivity();
    if (!state.yourTurn && state.aiLevel !== "off" && !state.isOnline) setTimeout(aiPlay, 800);
  }
}
...
function chooseHeuristic(moves, opts) {
  opts = opts || {};
